    ...
    if os.path.exists("usuarios.txt"):
        with open("usuarios.txt", "r") as file:
            for linea in file:
                ...
                historial = eval(peliculas)  # Convertir el string a lista
                visitas = len(historial)  # Calcula visitas basadas en entradas únicas en el historial
                    ...
                    if accion == "1":
                        ...
                        while True:
                            ...
                            if seleccion.isdigit() and 1 <= int(seleccion) <= len(peliculas):
                                ...
                                boletos_por_pelicula.append((pelicula, boletos))  # Registrar la película como entrada nueva
                            else:
                                print("Selección inválida. Intenta nuevamente.")
                        
                        if boletos_por_pelicula:
                            for pelicula, boletos in boletos_por_pelicula:
                                usuarios[username]["historial"].append((pelicula, boletos))  # Agregar entrada al historial
                            
                            usuarios[username]["boletos"] += sum(boletos for _, boletos in boletos_por_pelicula)
                            usuarios[username]["visitas"] += len(boletos_por_pelicula)  # Incrementar visitas por entradas nuevas
                            ...
                            otra_compra = input("¿Deseas comprar boletos para otra película? (si/no): ").lower()
                            if otra_compra != "si":
                        ...
                        if usuarios[username]["historial"]:
                            for pelicula, boletos in usuarios[username]["historial"]:
