...
def cargar_usuarios():
    ...
    if os.path.exists("usuarios.txt"):
        with open("usuarios.txt", "r") as file:
            for linea in file:
                username, password, boletos, peliculas, visitas = linea.strip().split(",")
                historial = eval(peliculas)  # Convertir el string a diccionario
                # Recalcular visitas según las películas únicas en el historial
                visitas_recalculadas = len(historial)
                usuarios[username] = {"password": password, "boletos": int(boletos), "historial": historial, "visitas": visitas_recalculadas}
    return usuarios
...
def guardar_usuarios(usuarios):
    with open("usuarios.txt", "w") as file:
        for username, data in usuarios.items():
            file.write(f"{username},{data['password']},{data['boletos']},{data['historial']},{data['visitas']}\n")
                ...
                while True:
                    ...
                    if accion == "1":
                        ...
                        if boletos_por_pelicula:
                            ...
                            # Calcula las películas nuevas para incrementar las visitas correctamente
                            nuevas_visitas = sum(1 for pelicula in boletos_por_pelicula if pelicula not in usuarios[username]["historial"])
                            usuarios[username]["visitas"] += nuevas_visitas  # Solo añade las películas nuevas como visitas
